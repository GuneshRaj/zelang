...
Student* Student_create(char* name, char* class) {
    char sql[1024];
    sprintf(sql, "INSERT INTO students (name, class) VALUES (?, ?)");
    ...
    sqlite3_bind_text(stmt, 2, class, -1, SQLITE_TRANSIENT);
...
int main(int argc, char *argv[]) {
    ...
    Student* student1 = Student_create("John Doe", "Class A");
    if (student1) printf("  Created Student with ID: %lld\n", student1->id);

    Student* student2 = Student_create("Jane Smith", "Class B");
    if (student2) printf("  Created Student with ID: %lld\n", student2->id);

    Student* student3 = Student_create("Bob Johnson", "Class A");
